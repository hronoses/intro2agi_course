...
class RerunViz:
    ...
    def __init__(self, app_id: str = 'agi_sim'):
        ...
        rr.log('metrics/coverage',     rr.SeriesLines(names=['Coverage %'],     colors=[(100, 200, 100)]), static=True)
        rr.log('metrics/object_count', rr.SeriesLines(names=['Object count'],   colors=[(100, 180, 255)]), static=True)
    ...
    def _log_scalars(self, model):
        rr.log('metrics/coverage',     rr.Scalars([model.coverage * 100]))
        rr.log('metrics/object_count', rr.Scalars([len(model.objects)]))
